...
- **悬浮播放控件与媒体卡片**：开启悬浮播放控件后，可选择在歌词区内嵌显示「上一首 / 播放暂停 / 下一首」，或在任务栏外展开独立媒体卡片；媒体卡片显示播放器图标/名称、封面、歌名/歌手、当前进度、总时长和进度条，支持纯色或磨砂玻璃背景、平滑展开/收起动画，点击播放器图标或名称可打开对应音乐软件
- **音频频谱**：跟随当前播放器（QQ音乐/网易云音乐，其他软件出声不受影响）播放跳动的圆角柱频谱，可在设置窗口开启/关闭；频谱颜色跟随已播放歌词颜色，开启时窗口自动加宽、关闭即恢复
- **性能模式**：在设置窗口切换「正常 / 低渲染 / 完全停止」；低渲染将播放动画限制在约 30 FPS 以降低 GPU/CPU 占用，完全停止会隐藏任务栏歌词、停止帧定时器并释放 GPU 资源，但保留当前会话数据，恢复后立即还原；两种省资源模式下频谱捕获自动暂停，切回正常模式后恢复
...
- **Fluent/Mica 对话框**：设置、字体选择、字体颜色与效果、颜色选择、手动搜索歌词和关于窗口统一采用 Windows 11 Fluent 风格，在支持的系统上使用 Mica 材质并跟随深色模式；相关对话框支持手动调整大小并保持最小尺寸与布局比例，高刷新率下拖动设置窗口时自动使用静态快照，减少拖动延迟
...
2. 右键托盘图标：
   ...
   - **退出**：退出程序
3. 在 **设置…** 窗口中：
   - **显示**：显示/隐藏歌曲信息、专辑封面和平台图标，选择默认或黑胶封面效果，开启/关闭频谱，选择正常/低渲染/完全停止性能模式，以及设置悬浮时是否显示播放控件、选择内嵌控件或媒体卡片和媒体卡片背景
   - **字体与颜色**：选择字体、字号和常规/加粗/斜体/粗斜体样式，调整歌词颜色、未播放歌词不透明度、光晕/描边，并设置已播放颜色是否跟随专辑
   - **歌词**：开启双行歌词，选择左对齐/居中/右对齐，开启翻译/罗马音并切换辅助歌词类型；启用 QQ 音乐本地歌词、选择本地歌词目录并设置切换版本持久化
4. 鼠标悬浮歌词区域可显示播放控制按钮（可在设置中选择关闭，默认开启）；选择「媒体卡片」时会展开独立卡片，显示播放器和歌曲播放进度，点击卡片顶部的播放器图标或名称可打开对应音乐软件
