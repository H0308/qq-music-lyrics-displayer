...
- **悬浮播放控件与媒体卡片**：开启悬浮播放控件后，可选择在歌词区内嵌显示「上一首 / 播放暂停 / 下一首」，或在任务栏外展开独立媒体卡片；媒体卡片显示播放器图标/名称、封面、歌名/歌手、当前进度、总时长和进度条，支持纯色或磨砂玻璃背景，磨砂背景可跟随专辑主色渐变并根据背景明暗自动适配文字颜色，展开/收起和切歌均有平滑过渡，点击播放器图标或名称可打开对应音乐软件
...
2. 右键托盘图标：
   ...
   - **退出**：退出程序
3. 在 **设置…** 窗口中：
   - **显示**：分别设置任务栏歌词与对话框/悬浮窗的系统、应用、浅色或深色主题，显示/隐藏歌曲信息、专辑封面和平台图标，选择默认或黑胶封面效果，选择正常/低渲染/完全停止性能模式，以及设置悬浮时是否显示播放控件、选择内嵌控件或媒体卡片和媒体卡片背景；磨砂背景还可选择跟随专辑主色和自动适配文字颜色
   - **频谱**：开启/关闭频谱，选择默认、柱状图或梦幻波浪样式；梦幻波浪可作为歌曲信息和歌词背景，并调节背景透明度
...
4. 鼠标悬浮歌词区域可显示播放控制按钮（可在设置中选择关闭，默认开启）；选择「媒体卡片」时会展开独立卡片，显示播放器和歌曲播放进度，点击卡片顶部的播放器图标或名称可打开对应音乐软件，切歌时卡片内容平滑过渡
