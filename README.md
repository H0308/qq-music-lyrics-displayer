...
- **切歌浮窗提示**：切歌时在主屏幕中下方（可选中上方）短暂弹出封面、歌名和歌手；弹窗磨砂半透明、置顶且鼠标穿透，依据背后内容明暗自动适配文字颜色，滑入滑出平滑过渡；可设置停留时长（1~10 秒）、弹出位置，以及前台有全屏应用时不弹出
...
- **Fluent/Mica 对话框**：设置、字体选择、字体颜色与效果、颜色选择、手动搜索歌词和关于窗口统一采用 Windows 11 Fluent 风格，在支持的系统上使用 Mica 材质；任务栏歌词与对话框/悬浮窗可分别跟随系统、跟随应用或固定为浅色/深色，相关对话框支持手动调整大小并保持最小尺寸与布局比例，高刷新率下拖动设置窗口时自动使用静态快照，减少拖动延迟
...
- **运行日志**：托盘菜单可打开运行日志面板，查看当前播放、歌词来源、封面加载、CPU/GPU 与专用内存等状态；详细运行事件（包括资源创建与释放）按会话写入日志文件，并可选择保存目录和定期清理周期
...
2. 右键托盘图标：
   ...
   - **运行日志**：查看当前播放、歌词来源、封面加载和资源占用状态，打开日志目录或设置日志保留周期
   - **退出**：退出程序
3. 在 **设置…** 窗口中：
   - **显示**：按「主题 / 任务栏歌词 / 悬浮控件与媒体卡片 / 切歌弹窗 / 性能」分组；分别设置任务栏歌词与对话框/悬浮窗的系统、应用、浅色或深色主题，显示/隐藏歌曲信息、专辑封面和平台图标，选择默认或黑胶封面效果，选择任务栏歌词背景（无 / 封面模糊 / 纯色）并调节封面模糊不透明度，开启播放进度背景并调节不透明度，选择正常/低渲染/完全停止性能模式；设置悬浮时是否显示播放控件、选择内嵌控件或媒体卡片、媒体卡片展开方式（悬浮/点击）和媒体卡片背景，磨砂背景还可选择跟随专辑主色和自动适配文字颜色；设置切歌弹窗开关、停留时长、位置和全屏时是否弹出
